...
final float sAB(float A[],float B[]){
float s=0;
...
float rA = sqrt(sAB(A,A));  // projection A on B
float rB = sqrt(sAB(B,B));  // length of B
toAcB(A,B,rA/rB);
}
...
float rA = sqrt(sAB(A,A));  // length of A
float rB = sqrt(sAB(B,B));  // length of B
float  c = sAB(A,B);  // projection A on B
return ( c/(rB*rA));
}
