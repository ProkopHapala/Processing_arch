/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/121712*@* */
...
//    Data[i] = new PVector( 200+r*cos(i*PI/180),200+ r*sin(i*PI/180));
  ...
  for(int i =0;  i<m;i++){  Data[i] = new PVector( random(500), random(400) ); }
  /*
...
void drawparticles(){
  ...
  for(int i=0;i<gbest.cps.length;i++)  ellipse(gbest.cps[i].x,gbest.cps[i].y,5,5);
...
public class Position{
  ...
  private void initialize_position()  {
   ...
   public void update_position(Velocity velocity)   {
      for(int i=1;i<cps.length-1;i++)
      {
         cps[i].x=cps[i].x+velocity.cpsv[i-1].x*dt;
         cps[i].y=cps[i].y+velocity.cpsv[i-1].y*dt;
      }
