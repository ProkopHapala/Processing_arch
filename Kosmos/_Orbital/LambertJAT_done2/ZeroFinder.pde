...
public class ZeroFinder {
  private int    maxit      = 500;        // maximum number of iterations
  private double accuracy   = 1.0E-12;    // accuracy of the function, how close to f = 0 
  private double dxmin      = 1.0E-12;    // accuracy of the solution 
  private double UNUSED     = -1.11E+30;  // bad number
  private int err           = 0;          // error counter
  private ScalarFunction func;            //  the function to be solved 
  ...
  public ZeroFinder(ScalarFunction f) { this.func = f;  }
  ...
  public void setMaxIterations(int max) {    this.maxit = max;     }
  public void setAccuracy(double acc)   {    this.accuracy = acc;  }
  public void set_dxmin(double x)       {    this.dxmin = x;       }
  ...
  public boolean zbrac(double x1, double x2){
    ...
    boolean out = false;
    if (x1 == x2) {
      println("You must provide ZBRAC with an interval");
      return out;
    }
    ...
    println("ZBRAC: Failed to find a bracket in "+ntry+" attempts");
    out = false;
    return out;
  }
...
//    }
  ...
  public double regulaFalsi(double x1, double x2) {
    ...
    if (test > 0.0) {
      ...
      if (fh == 0.0) return x2;
      err++;
      println("Root must be bracketed in ZeroFinder "+err);
    }
    if (fl < 0.0) {
      xlow = x1;
      xhigh = x2;
    ...
    for (int i = 1; i < this.maxit; i++) {
      ...
      dx = xhigh - xlow;
      if ((Math.abs(del) < this.dxmin) || (Math.abs(f) < this.accuracy)) {
        converged = true;  
        return out;
      }
    }

    //System.out.println( " Regula Falsi exceeded " + this.maxit + " iterations ");
    return out;
  }
  ...
  public double fixedPtIteration(double x0, double dum) {
    ...
    System.out.println(      " Fixed Point Iteration Method exceeded " + this.maxit + " iterations ");
    return 0.0;
  }
