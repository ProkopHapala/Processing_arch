...
public class NavierStokesSolver {
	...
	double [][] diverg = new double[N+2][N+2];
 ...
 // ---------------------- MAIN ALGORITHM ------------------------
  //---------------------------------------------------------------
	...
	final void vel_step(double[][] vx, double[][] vy, double[][] vx0, double[][] vy0, double visc, double dt) {
                fillv(vx0,0.0); fillv(vy0,0.0);
		add_source(vx, vx0, dt);                add_source(vy, vy0, dt);
		replace (vx0, vx);                      replace(vy0, vy);
		diffuse(vx, vx0, visc, dt); 		diffuse(vy, vy0, visc, dt);
                PBC(vx);                         PBC(vy);
		project(vx, vy, p, diverg);
                replace(vx0, vx);      		        replace(vy0, vy);
		advect(vx, vx0,     vx0, vy0, dt);      advect(vy, vy0,     vx0, vy0, dt);
                PBC( vx);                PBC( vy);   
		project(vx, vy, p, diverg);             // meaning of variables:      void project(vx, vy, p, diverg)
	}


       final void replace(double[][] A, double[][] B) {
...
*/
	...
	final void PBC(double[][] x) {    // periodic boundary condition
              for (int i = 1; i <= N; i++) { 
                x[0  ][i] =  x[N][ i];
		x[N+1][i] =  x[1][ i];
                x[i][0  ] =  x[i][N ];
	...
	final void fillv(double[][] x, double val) {    // periodic boundary condition
           for (int ix = 0; ix <= N+1; ix++) { 
              for (int iy = 0; iy <= N+1; iy++) { 
                x[ix][iy] =  val;
              }    }
