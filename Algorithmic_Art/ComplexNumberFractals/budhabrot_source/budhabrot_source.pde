...
int bailout =1000;         // number of iterations before bail
int plots = 100000;        // number of plots to execute per frame (x30 = plots per second)

// 2D array to hold exposure values
float[] exposure = new float[dim*dim];
...
void plotPlots() {
  ...
  for (int n=0;n<plots;n++) {
    ...
    if (iterate(x,y,false)) {
      iterate(x,y,true);
      /*
      int ix = int(dim * (x + 2.0) / 3.0);
      int iy = int(dim * (y + 1.5) / 3.0);
