...
float xmin = random(sz);       float ymin = random(sz);
float xmax = random(xmin,sz);  float ymax = random(ymin,sz);
...
void setup(){
 ...
 for (int i=0;i<1000;i++){
  /* 
   ...
   constrain (o.x + random(-10,10)   , 0,  myTree.Lx ) ,
   constrain (o.y + random(-10,10)   , 0,  myTree.Ly ) 
   );
 */
