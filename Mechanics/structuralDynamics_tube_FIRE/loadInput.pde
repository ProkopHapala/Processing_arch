...
void loadLinks( String fname){
  ...
  for (TableRow row : table.rows()) {
    links[i]=new Link( 
      bodies[int(row.getString("ia").trim())], bodies[int(row.getString("ib").trim())], materials.get(row.getString("material").trim()), row.getFloat("area"), row.getFloat("r0")
    );
    i++;
  }
}
...
void loadBodies( String fname){
  Table table = loadTable(fname, "header");
  println(table.getRowCount() + " rows in "+fname);
  bodies = new BasicBody[ table.getRowCount() ];
  int i=0;
  for (TableRow row : table.rows()) {
    boolean fixed = false;
    //if( row.getString("fixed").trim().charAt(0)=='T' ) fixed = true;
    if( row.getFloat("fixed")>0 ) fixed = true;
    bodies[i]=new BasicBody( 
      row.getFloat("mass"), new double3(row.getFloat("x"), row.getFloat("y"),row.getFloat("z")), fixed,
      new double3(row.getFloat("vx"), row.getFloat("vy"),row.getFloat("vz"))
    );
    i++;
  }
}
