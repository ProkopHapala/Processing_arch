...
#define Type 5
float U = 0.0*cos(time)*0.5+0.1;
float V =  0.2*sin(time*0.1)*0.5+0.2;
float W =  1.0*cos(time*1.2)*0.5+0.5;
float T =  1.0;

float VRadius = 0.05048;
float SRadius = 0.05476;
vec3 RotVector = vec3(0.0,1.0,1.0);
float RotAngle = time*0.0;


mat3 rot;
vec4 nc,nd,p;
void init() {
	U = mouse.x/resolution.x;
	W = 1.0-U;
	V = mouse.y/resolution.y;
	T = 1.0-V;
