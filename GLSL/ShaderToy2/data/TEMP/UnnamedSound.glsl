...
vec2 noise2(vec2 p)
{
	return 
		vec2(
			noise(vec3(p, 1.9+sin(time*0.8)*1.3)), 
			noise(vec3(p, -1.2+sin(time*1.2)*1.0)));
}
...
vec2 noise2(vec2 p, float fudge)
{
	return 
		vec2(
			noise(vec3(p, fudge+sin(time*0.8)*1.3)), 
			noise(vec3(p, -fudge+sin(time*1.2)*1.0)));
}
...
vec2 pan_rotate_zoom(vec2 pos, vec4 val)
{
	...
	float angle= pi*val.y+timeEffect*(sin(time+1.2)-1.0);
	float zoom = val.z;
...
vec4 imageFunction(vec2 pos)
{
	...
	float p3 = texture2D( iChannel0, vec2(0.7,0.0) ).x * 0.0;
	float p4 = texture2D( iChannel0, vec2(1.0,0.0) ).x * 0.0;
	pos = pos * (1.5 + sin(time*0.05+origPos.x)*0.35+p1*0.2);
	pos = array(pos);
	vec4 rot = sinf(x_y_ang_dist(rotate(mirror(pos), time*0.05+p0*0.5)));	
	pos = rotate(pos, rot+origPos.y);
	return x(rotate(julia(pos), time*0.2+p2*0.0));
}
