...
vec4 imageFunction(vec2 pos)
{
	...
	pos = pos * (1.0 + sin(time*0.2)*0.2);
	pos = kaleidoscope(array(pos))*(1.2+sin(time*f*2.0)*0.75);	
	pos = rotate(pos, time*f);

	vec4 col=triangleWave(x(pos));
	
	
	//return ridged_bw(rotateAndMove(pos, time*0.01),time*0.2);
	return col;
